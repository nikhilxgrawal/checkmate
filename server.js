...
async function saveDB(db) {
  ...
  try {
    ...
    for (const m of db.matches) {
      await conn.query(
        `INSERT INTO matches (id, gameId, tournamentId, playerAId, playerBId, time, day, location, status, result, winnerId, startAt, endAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
app.get("/api/events", (req, res) => {
  ...
  req.on("close", () => {
    ...
    console.log(`[sse] client closed (open=${sseClients.size})`);
  });
});
...
async function runScheduler() {
  ...
  try {
    const now = Date.now();
    // Cheap read-only pre-check: only take the write lock (a full DB rewrite)
    // if at least one match actually needs to flip. Without this the scheduler
    // rewrote the whole dataset every 20s and contended with page-load reads
    // and bids on the global write chain — a source of intermittent lag.
    const pre = await loadDB();
    const needsFlip = pre.matches.some((m) =>
      !isDecided(m) && (
        (m.status === "upcoming" && m.startAt && now >= m.startAt) ||
        (m.status === "live" && m.endAt && now >= m.endAt)
      )
    );
    if (!needsFlip) return;
...
(async () => {
  try {
    await ensureSchema();          // create/alter any missing tables & columns
